...
# biped ---------------------------------------------------------------------------------------------------
    ...
    def buildBiped_cmd(self):  # biped
        ...
        self.switchNullSet('L')
        self.switchNullSet('R')
        #if ui.bipedRigType_CMB.currentText() =='Soft Stretch IK':
        #    self.softStretchIK_set_cmd()
    ...
    def softStretchIK_set_cmd(self):
        ssIK = softStretchIKSet()
        ssIK.setSoftIK()

    def switchNullSet(self,side):
        footSnap = cmds.group(n='%s_FK_foot_snapSpace_NUL' %side,w=1,em=1)
        handSnap = cmds.group(n='%s_FK_hand_snapSpace_NUL' %side,w=1,em=1)
        cmds.matchTransform(footSnap ,'%s_FK_foot_JNT' %side , pos=True)
        cmds.matchTransform(handSnap ,'%s_FK_hand_JNT' %side , pos=True)
        cmds.parent(footSnap, '%s_FK_foot_JNT' %side)
        cmds.parent(handSnap, '%s_FK_hand_JNT' %side)
...
# quadruped ---------------------------------------------------------------------------------------------------
    ...
    def finishOP( self, *args ):
        ...
        self.quadAddLocalSpace()

    def quadAddLocalSpace(self):
        ' add local Space head, neck, tail !! '
        addLocalSpace = spaceBlend()
        cmds.delete( 'root_NUL' )
    ...
    def etc_set( self ):
        ...
        if self.ui.advTailNum_CMB.currentText() == 'Normal':
            ...
            cmds.parent ( 'C_Skin_tail1_JNT', 'SkinJoint_GRP' )

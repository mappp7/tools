...
class quadrupedRigClass:
    ...
    def importTemplateJoint( self, *args ):
        ...
        root_joint = cmds.joint ( p=( 0, 0, 0 ), n='template_tail1_JNT' )

        for x in range(int(tailJointNum)-1):
            cJoint = cmds.joint ( p=(0, 0, 0), n='template_tail%s_JNT' %(x+2) )
            pJoint = cmds.listRelatives ( p=1 )
            cmds.joint ( pJoint, e=1, zso=1, oj='xyz', sao='yup')
            cmds.select ( cJoint )
        
        cmds.select ( 'template_tail1_JNT', hi=1 )
        tailJointList = cmds.ls ( sl=1 )
        
        for x in tailJointList:
            cmds.setAttr ( x+'.tx', 2 )
            
        cmds.setAttr ( 'template_tail1_JNT.jointOrientY', 90 )
        cmds.setAttr ( 'template_tail1_JNT.tx', 0 )
        cmds.setAttr ( 'template_tail1_JNT.ty', 15 )
        cmds.setAttr ( 'template_tail1_JNT.tz', -11 )
    ...
    def create_skin_joint( self ):
        ...
        tempJoint_rootList = [ 'R_template_clavicle_JNT','C_template_root_JNT','C_template_neck1_JNT','L_template_clavicle_JNT','L_template_hip_JNT','R_template_hip_JNT' ]

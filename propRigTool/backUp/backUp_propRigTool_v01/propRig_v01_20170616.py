...
def sameNameCheck():
	...
	for i in range(len(allObjects)):
	    try:
			objectInSel=allObjects[i].split('|')
			if objectInSel[1] not in objectsWithExtra:
				objectsWithExtra.append(objectInSel[1])
				print allObjects[i]
				cmds.select( allObjects[i], add=True )
	    except:
	        pass

	if not objectsWithExtra:
		print 'same name OK!!'
		cmds.button('NameBtn', e=1, bgc=[.15,.5,.15])

# group NameCheck #
def groupNameCheck():
    cmds.select(cl=1)
    myallObjs = cmds.ls(type='transform')
    myCam = cmds.ls(type='camera')
    myShape = cmds.ls(type='mesh')

    myCamList = []
    for i in range(len(myCam)):
        extra = cmds.listRelatives(myCam[i],p=1)[0]
        myCamList.append(extra)
        myallObjs.remove(myCamList[i])

    myObjs = cmds.filterExpand(myShape,sm=12)
    for x in range(len(myObjs)):
        myallObjs.remove(myObjs[x])
    ...
    for w in range(len(myGrp)):
        if myGroup != myGrp:
            myGroup.remove(myGrp[w])
            cmds.select(myGroup)
        elif myGroup == myGrp:
            cmds.button('groupBtn', e=1, bgc=[.15,.5,.15])
            		
# *_PLY Check #
def _polyCheck():
    cmds.select(cl=True)
    cmds.select('*_PLY')
    nameMesh = cmds.ls(sl=1)
    cmds.select(cl=True)

    objShape = cmds.ls(type='mesh')
    myallObjs = cmds.filterExpand(objShape, sm=12)

    for i in range(len(nameMesh)):

        if nameMesh == myallObjs:
            print 'ok'
            cmds.button('_polyBtn', e=1, bgc=[.15,.5,.15])

        elif nameMesh != myallObjs:
            myallObjs.remove(nameMesh[i])
            cmds.select(myallObjs)	

# mesh ChannelBox Check #
def Channel_Check():
    cmds.select(cl=True)
    myShape = cmds.ls(type='mesh')
    myObjs = cmds.filterExpand(myShape, sm=12)
    ...
    for i in range(len(myObjs)):
        extra1 = cmds.getAttr('%s.translate'%myObjs[i])
        myTrans.append(extra1)
        extra2 = cmds.getAttr('%s.rotate'%myObjs[i])
        myRotate.append(extra2)
        extra3 = cmds.getAttr('%s.scale'%myObjs[i])
        myScale.append(extra3)


        if myTrans[i] == [(0,0,0)]:
            print 'good'
            myTransList.append('good')

        elif len(myTransList) != len(myObjs):
            print myObjs[i]
            cmds.select(myObjs[i],add=True)

        if myRotate[i] == [(0,0,0)]:
            print 'good'
            myRotateList.append('good')
        elif len(myRotateList) != len(myObjs):
            cmds.select(myObjs[i],add=True)

        if myScale[i] == [(1,1,1)]:
            print 'good'
            myScaleList.append('good')
        elif len(myScaleList) != len(myObjs):
            cmds.select(myObjs[i],add=True)

    if len(myTransList) == len(myRotateList) == len(myScaleList) == len(myObjs):
        cmds.button('channeltBtn', e=1, bgc=[.15,.5,.15])

# mesh_pivotCheck #
def pivotCheck():

    cmds.select(cl=True)
    myObjshape = cmds.ls(type='mesh')
    myObjs = []
    List = []
    test = []

    for i in range(len(myObjshape)):
        extList = cmds.listRelatives(myObjshape[i],ap=1 )[0]
        myObjs.append(extList)

    for x in range(len(myObjs)):
        pos= cmds.xform(myObjs[x], ws=1, rp=1, q=1)
        List.append(pos)
        ...
        if pos != [0,0,0]:
            cmds.select( myObjs[x], add=True )

        elif len(test) == len(myObjs):
            cmds.button('pivotBtn', e=1, bgc=[.15,.5,.15])

# mesh historyCheck #
def historytCheck():

    cmds.select(cl=True)
    extraList = cmds.ls(type='mesh')
    myObjs = cmds.filterExpand(extraList,sm=12)

    myList = []
    test = []

    for i in range(len(myObjs)):
        extraList2 = cmds.listRelatives(myObjs[i])
        myList.append(extraList2)
    ...
    for x in range(len(myList)):
        ...
        if len(myList) == len(test):
            cmds.button('historytBtn', e=1, bgc=[.15,.5,.15])

        elif len(myList[x]) >= 2:
            cmds.select(myList[x],add=True)
            asd = cmds.ls(sl=1)
            zxc = cmds.filterExpand(asd,sm=12)
            cmds.select(cl=1)
            cmds.select(zxc)
#                                                                                                                     #
#                                                                                                                     # 
...
def Create_propRig_group():    
    ...
    #__direction__#
    directionCON = cmds.circle (nr=(0, 1, 0), c=(0, 0, 0), r=(5.25), n='direction_CON')
    ...
    #__move__#
    moveCON = cmds.circle (nr=(0, 1, 0), c=(0, 0, 0), r=(4.5), n='move_CON')
...
def boundingBox_Info () :
    ...
    bbX_CT_TP = (bbInfo_TP[0] + bbInfo_TP[3]) / 2.0; bbX_CT.append (bbX_CT_TP)
    bbY_CT_TP = (bbInfo_TP[1] + bbInfo_TP[4]) / 2.0; bbY_CT.append (bbY_CT_TP)
    bbZ_CT_TP = (bbInfo_TP[2] + bbInfo_TP[5]) / 2.0; bbZ_CT.append (bbZ_CT_TP)
    bbsX_TP = cmds.getAttr ("%s.boundingBoxSizeX" % sel_temp[0]) / 1.5; bbsX.append(bbsX_TP)
    bbsY_TP = cmds.getAttr ("%s.boundingBoxSizeY" % sel_temp[0]) / 1.5; bbsY.append(bbsY_TP)
    bbsZ_TP = cmds.getAttr ("%s.boundingBoxSizeZ" % sel_temp[0]) / 1.5; bbsZ.append(bbsZ_TP)
...
def create_JNT_separate_CT () :
    ...
    for b in sepaJNT_selPLY_CT :
        ...
        cmds.move (0, sepaJNT_bbCT_Y_CT, 0, "%s.scalePivot" %sepaJNT_NUL_CT, "%s.rotatePivot" %sepaJNT_NUL_CT)
        cmds.move (0, sepaJNT_bbCT_Y_CT, 0, "%s.scalePivot" %sepaJNT_CON_rename_CT, "%s.rotatePivot" %sepaJNT_CON_rename_CT)
...
def create_JNT_combine () :
    ...
    cmds.select (deselect=True)
    # create joint
    combJNT = cmds.joint (p=(bbX_CT[-1], bbY_CT[-1], bbZ_CT[-1]), n=combJNT_selPLY[0].replace('PLY', 'JNT'))
    ...
    cmds.scale (bbsX[-1], bbsY[-1], bbsZ[-1], combJNT_NUL)
    cmds.move (bbX_CT[-1], bbY_CT[-1], bbZ_CT[-1], combJNT_NUL)  
...
def create_JNT_combine_CT () :
    ...
    cmds.select (deselect=True)
    # create joint
    combJNT_CT = cmds.joint (p=(bbX_CT[-1], bbY_CT[-1], bbZ_CT[-1]), n=combJNT_selPLY_CT[0].replace('PLY', 'JNT'))
    ...
    cmds.scale (bbsX[-1], bbsY[-1], bbsZ[-1], combJNT_NUL_CT)
    cmds.move (bbX_CT[-1], bbY_CT[-1], bbZ_CT[-1], combJNT_NUL_CT)
    cmds.move (0, bbY_CT[-1], 0, "%s.scalePivot" %combJNT_NUL_CT, "%s.rotatePivot" %combJNT_NUL_CT)
    cmds.move (0, bbY_CT[-1], 0, "%s.scalePivot" %combJNT_CON_rename_CT, "%s.rotatePivot" %combJNT_CON_rename_CT)
...
def create_CON_separate_CT () :
    ...
    for e in sepaCON_selPLY_CT :
        ...
        cmds.move (0, sepaCON_bbCT_Y_CT, 0, "%s.scalePivot" %sepaCON_NUL_CT, "%s.rotatePivot" %sepaCON_NUL_CT)
        cmds.move (0, sepaCON_bbCT_Y_CT, 0, "%s.scalePivot" %sepaCON_rename_CT, "%s.rotatePivot" %sepaCON_rename_CT)   
...
def create_CON_combine () :
    ...
    cmds.scale (bbsX[-1], bbsY[-1], bbsZ[-1], combCON_NUL)
    cmds.move (bbX_CT[-1], bbY_CT[-1], bbZ_CT[-1], combCON_NUL)
...
def create_CON_combine_CT () :
    ...
    boundingBox_Info ()
    cmds.scale (bbsX[-1], bbsY[-1], bbsZ[-1], combCON_NUL_CT)
    cmds.move (bbX_CT[-1], bbY_CT[-1], bbZ_CT[-1], combCON_NUL_CT)
    cmds.move (0, bbY_CT[-1], 0, "%s.scalePivot" %combCON_NUL_CT, "%s.rotatePivot" %combCON_NUL_CT)
    cmds.move (0, bbY_CT[-1], 0, "%s.scalePivot" %combCON_rename_CT, "%s.rotatePivot" %combCON_rename_CT)    
...
def create_rootCON_CT () :
    ...
    cmds.move (0, tempGRP_bbInfo_Y_CT, 0, "%s.scalePivot" % rootNUL_CT, "%s.rotatePivot" % rootNUL_CT)
    cmds.move (0, tempGRP_bbInfo_Y_CT, 0, "%s.scalePivot" % rootCON_rename_02_CT, "%s.rotatePivot" % rootCON_rename_02_CT) 
...
sr_window = 'propRig_v01'                                                                                             #
if cmds.window (sr_window, q=1, ex=1):                                                                                #
    cmds.deleteUI (sr_window)    

cmds.window (sr_window, h=50)

#__________ 1. Model Check __________#
cmds.columnLayout (adj=1, bgc=([0.2, 0.2, 0.2]), parent = sr_window)
cmds.frameLayout (l='Model Check', collapsable=1, collapse=1, bgc=[0.7, 0.3, 0.3])
#cmds.frameLayout (l='Model Check', collapsable=1, collapse=1, bgc=[0.2, 0.2, 0.2])
cmds.separator (st='single')
#cmds.text ('1. MODEL CHECK', font='obliqueLabelFont')
cmds.text ('1. MODEL CHECK', font='obliqueLabelFont', h=20, bgc=([1, 0.8, 0.8]))
cmds.separator (st='single')
cmds.rowColumnLayout (numberOfColumns=2, columnWidth = [(1,142), (2,142)])
cmds.button ('NameBtn', l='1. Name (same)', h=30, c='sameNameCheck()')
cmds.button ('channeltBtn', l='4. Channel Box', h=30, c='Channel_Check()')
cmds.button ('groupBtn', l='2. Name (GRP)', h=30, c='groupNameCheck()')
cmds.button ('pivotBtn', l='5. Pivot', h=30, c='pivotCheck()')
cmds.button ('_polyBtn', l='3. Name (PLY)', h=30, c='_polyCheck()')
cmds.button ('historytBtn', l='6. History', h=30, c='historytCheck()', al="right")
...
cmds.text ('2. CREATE dxRig NODE', font='obliqueLabelFont', h=20, bgc=([1, 0.87, 0.76]))
cmds.separator (st='single', bgc=([0.5, 0.5, 0.1]))
cmds.text ('YOU MUST SELECT SOMETHING !')

cmds.rowColumnLayout (numberOfColumns=3, columnWidth=[(1,50), (2,180), (3,50)])
cmds.text (" ")
cmds.button (l='Create Rig GRP', w=150, h=30, bgc=([1, 0.6, 0.1]), c = 'Create_propRig_group()')
cmds.text (" ")
...
cmds.columnLayout (adj=1, h=115, p=color_frameLayout)
...
# window GUI : Size
cmds.window (sr_window, e=1, width=290, sizeable=1)
cmds.showWindow(sr_window)                                                                                            #
#                                                                                                                     # 
#                                                                                                                     # 
#                                                                                                                     # 
###################################################    Window GUI    ##################################################      

...
def create_subCON () :
    selCON = cmds.ls(sl=1)
    selCON_nameList = selCON[0].split ('_')
    selCON_nameList_search = selCON_nameList[-2]
    a = selCON_nameList_search[:3]
    if a.count('sub') is 0 :
        copyMainCON ()    
    else :
        check_createSecond_subCON ()
        
    
        
def copyMainCON () :
    selCON = cmds.ls(sl=1)   
    selMainCON_copy = cmds.duplicate (selCON[0], n=selCON[0].replace ('CON', 'sub1_CON')); cmds.select (selMainCON_copy[0])
    subCON = cmds.ls(sl=1)
    cmds.setAttr ("%s.sx" %subCON[0], k=1, l=0)
    cmds.setAttr ("%s.sy" %subCON[0], k=1, l=0)
    cmds.setAttr ("%s.sz" %subCON[0], k=1, l=0)
    subCON_NUL = cmds.group(em=1, n=subCON[0].replace ('CON', 'NUL'))
    cmds.parent (subCON_NUL, selCON[0])
    cmds.setAttr ("%s.tx" %subCON_NUL, 0)
    cmds.setAttr ("%s.ty" %subCON_NUL, 0)
    cmds.setAttr ("%s.tz" %subCON_NUL, 0)
    cmds.parent (subCON[0], subCON_NUL)
    cmds.setAttr ("%s.sx" %subCON_NUL, 0.9)
    cmds.setAttr ("%s.sy" %subCON_NUL, 0.9)
    cmds.setAttr ("%s.sz" %subCON_NUL, 0.9)
    cmds.makeIdentity (subCON_NUL, apply=1, s=1)
    cmds.setAttr ("%s.sx" %subCON[0], k=0, l=1)
    cmds.setAttr ("%s.sy" %subCON[0], k=0, l=1)
    cmds.setAttr ("%s.sz" %subCON[0], k=0, l=1)
    cmds.addAttr (selCON[0], longName='subConVis', at='bool', k=1, defaultValue=1.0)
    cmds.connectAttr("%s.subConVis" %selCON[0], "%s.visibility" %subCON_NUL)
    ''' 
    < short name >
    SCL = scaleConstraint List
    PCL = parentConstraint List
    '''
    selMainCON_connectedSCL = cmds.listConnections (selCON[0], s=1, t="scaleConstraint")
    selMainCON_connectedPCL = cmds.listConnections (selCON[0], s=1, t="parentConstraint")
    ...
    for a in range(len(selPLY)) :
        cmds.parentConstraint (subCON[0], selPLY[a], mo=1)
    cmds.select (deselect=1)


def check_createSecond_subCON () :
...
def copySubCON () :
    selCON = cmds.ls(sl=1)
    selCON_rename = cmds.rename (selCON[0], selCON[0].replace('_CON',''))
    ...
    PCL = parentConstraint List
    '''
    selSubCON_connectedSCL = cmds.listConnections (selCON[0], s=1, t="scaleConstraint")
    selSubCON_connectedPCL = cmds.listConnections (selCON[0], s=1, t="parentConstraint")
...
def propRigWindow () :
    ...
    cmds.rowColumnLayout (numberOfColumns=3, columnWidth=[(1,50), (2,180), (3,50)])
    cmds.text (" ")
    cmds.button (l='Create !', w=150, h=30, bgc=([1, 0.87, 0.76]), c = 'Create_propRig_group()') 
    cmds.text (" ")
    ...
    cmds.separator (st='none', h=10)
    cmds.text ('<    subCON    >        :')
    cmds.button (l='Create', h=30, bgc=[0.4, 0.4, 0.4], c='create_subCON ()')
    cmds.separator (st='none', h=5)
    cmds.separator (st='none', h=5)

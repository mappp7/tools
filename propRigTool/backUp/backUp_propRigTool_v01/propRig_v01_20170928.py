...
def create_subCON () :
    ...
    subCON_selCON = cmds.ls(sl=1)
    subCON_selCON_rename = cmds.rename (subCON_selCON[0].replace ('CON', 'sub_CON'))
    subCON_copyCON = cmds.duplicate (subCON_selCON_rename, n = subCON_selCON_rename.replace ('sub_CON', 'CON'))
    cmds.setAttr ("%s.sx" %subCON_copyCON[0], k=1, l=0)
    cmds.setAttr ("%s.sy" %subCON_copyCON[0], k=1, l=0)
    cmds.setAttr ("%s.sz" %subCON_copyCON[0], k=1, l=0)
    cmds.parent (subCON_selCON_rename, 'control_GRP')
    subCON_NUL = cmds.group(n=subCON_selCON_rename.replace ('CON', 'NUL'))
    subCON_copyCON_NUL = cmds.listRelatives (subCON_copyCON[0], parent=1)
    cmds.setAttr ( "%s.sx" %subCON_copyCON_NUL[0], 1.1)
    cmds.setAttr ( "%s.sy" %subCON_copyCON_NUL[0], 1.1)
    cmds.setAttr ( "%s.sz" %subCON_copyCON_NUL[0], 1.1)
    cmds.makeIdentity (subCON_copyCON_NUL, apply=1, s=1)
    cmds.setAttr ("%s.sx" %subCON_copyCON[0], k=0, l=1)
    cmds.setAttr ("%s.sy" %subCON_copyCON[0], k=0, l=1)
    cmds.setAttr ("%s.sz" %subCON_copyCON[0], k=0, l=1)
    cmds.parent (subCON_NUL, subCON_copyCON)
    cmds.addAttr (subCON_copyCON[0], longName ='subConVis', at='bool', k=1, defaultValue=1.0)
    cmds.connectAttr ("%s.subConVis" %subCON_copyCON[0], "%s.visibility" %subCON_NUL)
    cmds.select (deselect=1)
#                                                                                                                     #    
#                                                                                                                     #
#                                                                                                                     #

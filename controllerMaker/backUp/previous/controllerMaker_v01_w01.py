...
# buttonAction def Group
    def ButtonConnection(self):
        ...
        for i in range(1,33):
            eval( "self.color_%s_BTN.clicked.connect(self.controllerResize)" % i )
        self.FK_Control_Maker_BTN.clicked.connect(self.FKcontrolMaker)
                
    def overrideColor(self):
        self.number = 
        self.name = cmds.ls(sl=True)
        cmds.setAttr("%sShape.overrideEnabled" %name, 1)
        cmds.setAttr("%sShape.overrideColor" %name, number)

    def colourPallet(self):
        number = int(self.Do_Resize_DSB.text())
        overrideColor()
    ...
    def FKcontrolMaker(self):
        ...
        for i in sel:
            ...
            cmds.DeleteHistory(Controller[0])

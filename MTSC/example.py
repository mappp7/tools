...
def manual_codebook(mayaMesh,jointList):
    ...
    for i in range(num) :
        a = cmds.skinPercent(skincluster , mayaMesh + '.vtx[' + str(i) + ']' , q=1 , value =1 ) 
        arrayL.append(a.index(1.0))
...
def initialize(poses, rest_pose, num_bones, iterations, mayaMesh=None, jointList=None):
    ...
    if mayaMesh:
        ...
        boneArray = []
        for i in jointList :
            boneArray.append(cmds.xform(i , q=1 , t=1))
        
        rest_bones_t = np.array(boneArray)
        for bone in range(num_bones):
            rest_bones_t[bone] = np.array(boneList[bone])        
...
def SSDR(poses, rest_pose, num_bones, sparseness=4, max_iterations=20 , iterations = 5):
    """
    ...
    bone_transforms, rest_bones_t ,finalBone = initialize(poses, rest_pose, num_bones , iterations)
...
W, bone_transforms, rest_bones_t ,finalBone = SSDR(poses, rest_pose, 9)

'''
bonel = []
for i in rest_bones_t:
    cmds.select(cl=1)
    j = cmds.joint()
    cmds.xform(j , t=i)
    bonel.append(j)
...
sel_ = cmds.ls(sl=1)[0]
skincluster = mel.eval('findRelatedSkinCluster("%s")'%sel_)
...
for x in range(len(W)): 
    cmds.skinPercent(skincluster , 'pPlane8.vtx[%s]' %x , transformValue = tvdict[x] )

pn = 3
#pose name move
for i,na in enumerate(bonel):
    rotation = euler_angles_from_rotation_matrix(bone_transforms[i][pn][:3,:])
    trans = bone_transforms[i][pn][3,:]
    cmds.xform(na , t = trans , ro = rotation)

cmds.skinPercent('skinCluster2' , 'pPlane8' , pruneWeights = 0.01)

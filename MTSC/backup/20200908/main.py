...
class MtscTool(QtWidgets.QMainWindow):
    ...
    def initialize(self , poses, rest_pose, num_bones, iterations, mayaMesh=None, jointList=None):
        ...
        if mayaMesh:
            ...
            self.rest_bones_t = np.empty((num_bones , 3))                    # Translations for bones at rest pose

            for bone in range(num_bones):
                self.rest_bones_t[bone] = np.mean(rest_pose[vert_assignments == bone] , axis = 0)
                rest_pose_corrected[bone] = rest_pose - self.rest_bones_t[bone]
        ...
        for it in range(iterations):
            ...
            for bone in range(num_bones):
                Rp = bone_transforms[bone,:,:3,:].dot((rest_pose - self.rest_bones_t[bone]).T).transpose((0, 2, 1)) # |num_poses| x |num_verts| x 3
                # R * p + T
    ...
    def MTSC(self , poses, rest_pose, num_bones, iterations = 5 , max_iterations = 5 ,sparseness=4 ,mayaMesh = None , jointList = None):
        ...
        if mayaMesh and jointList:
            bone_transforms, self.rest_bones_t ,finalBone = self.initialize(poses, rest_pose, num_bones , iterations , mayaMesh , jointList)
        else:
            bone_transforms, self.rest_bones_t ,finalBone = self.initialize(poses, rest_pose, num_bones , iterations)
        ip = 90 / max_iterations
        
        for p in range(max_iterations):

            W = self.update_weight_map(bone_transforms, self.rest_bones_t, poses, rest_pose, sparseness)
            
            bone_transforms , self.rest_bones_t = self.update_bone_transforms(W, bone_transforms, self.rest_bones_t, poses, rest_pose ,self.mesh)
            print("Reconstruction error:", self.reconstruction_err(poses, rest_pose, bone_transforms, self.rest_bones_t, W))
        ...
        return W, bone_transforms, self.rest_bones_t ,finalBone
        ...
        if mode ==0 :
            if self.Weight:
                ...
                for p in range(max_iterations):
                    ...
                    bone_transforms , self.rest_bones_t = self.update_bone_transforms(WeightArray, self.boneTransform, self.rest_bone, poses, rest_pose ,self.mesh)
    ...
    def clickPoses(self , item):
        pose = self.ui.poses_LWG.currentRow()
        self.flipAxis(pose , self.Weight , self.rest_pose , self.poses , self.rest_bones_t  , self.mesh , self.finalBone)
        #rotation = self.matrix4x4_to_euler_xyz(self.boneTransform[i][row][:3,:])
        

    def flipAxis(self,pose,W,rest_pose,poses,rest_bones_t,mesh,finalBone):   
        rotation = []
        trans = []
        for bone in range(len(finalBone)):
            rotation = self.euler_angles_from_rotation_matrix(self.boneTransform[bone][pose][:3,:])
        ...
        def confirmFunc():
            ...
            for bone_ in range(num_bones):
                # When you are a vectorizing GOD
                constructions[bone_] = np.einsum('ijk,lk->ilj', self.boneTransform[bone_, :, :3, :], p_corrected[bone_]) # |num_poses| x |num_verts| x 3
            constructions += self.boneTransform[:, :, np.newaxis, 3, :] # |num_bones| x |num_poses| x |num_verts| x 3
            constructions *= (W.T)[:, np.newaxis, :, np.newaxis] # |num_bones| x |num_poses| x |num_verts| x 3
        ...
        while err > 0.1:
            ...
            for bone ,na in enumerate(finalBone):
                ...
                for i in range(len(finalBone)):
                    ...
                    lveXform = cmds.xform(mesh+'.vtx[%s]' %largest_vert_errs ,q=1 ,t=1)
                    dist_.append( math.sqrt(sum([(a - b) ** 2 for a, b in zip(trans, lveXform)])) )
    ...
    def updateBothFunc(self):
        ...
        W, bone_transforms, self.rest_bones_t ,self.finalBone = self.MTSC(self.poses, self.rest_pose ,num_bones ,iterations , maxIterations , sparseness ,self.mesh ,self.boneList)
        
        mel.eval("skinCluster -e  -ub %s"%self.mesh)
        for rbone in range(len(self.rest_bones_t)):
            cmds.xform(self.finalBone[rbone] , ws=1 ,t=self.rest_bones_t[rbone])

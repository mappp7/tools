...
def initialize(poses, rest_pose, num_bones ,iterations = 3 ,mayaMesh = None ,jointList = None):
    start_time = time.time()
    bone_transforms, rest_bones_t ,finalBone = initialize(poses, rest_pose, num_bones , iterations , mayaMesh , jointList)
    end_time = time.time()
    
    return bone_transforms , rest_bones_t , finalBone

def MTSC(mode , poses, rest_pose, bone_transforms , rest_bones_t , finalBone, sparseness=4 ,mayaMesh = None , jointList = None):

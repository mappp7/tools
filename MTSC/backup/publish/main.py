...
class MtscTool(QtWidgets.QMainWindow):
    ...
    def update_bone_transforms(self,W, bone_transforms, rest_bones_t, poses, rest_pose ,mesh ,epsilon =3):
        ...
        for pose in range(num_poses):
            for bone in range(num_bones):
                ...
                '''
                    ...
                    erra = np.linalg.norm(constructed - poses , axis=(1,3))
                    #vert_assignments = np.argmin(erra, axis=0)
                    ...
                    errs = erra[bone].tolist()
                    vtxNum = errs.index(erra[bone].max())
                    print vtxNum
                    # print vtxNum
                    pos = cmds.xform(mesh + '.vtx[' +  str(vtxNum) + ']' ,q=1,t=1,ws=1)
                    find_index = self.find_vtx_sixteen_Point( mesh , pos) # output : [1,3,8,12,15,18]
                    
                    bone_transforms[bone, pose] = self.kabsch(rest_pose_corrected[bone,find_index] , poses[pose , find_index])  
                '''
    ...
    def MTSC(self , poses, rest_pose, num_bones, iterations = 5 , max_iterations = 5 ,sparseness=4 ,mayaMesh = None , jointList = None):
        ...
        for _ in range(max_iterations):
            ...
            #bone_transforms = self.reInitialize_bone(self.mesh , W ,rest_pose, poses ,bone_transforms,rest_bones_t)
            bone_transforms = self.update_bone_transforms(W, bone_transforms, rest_bones_t, poses, rest_pose ,self.mesh)
    ...
    def reInitialize_bone(self ,mesh,W , rest_pose, poses ,bone_transforms,rest_bones_t,epsilon=3.0):
        num_bones = W.shape[1]
        num_poses = poses.shape[0]
        num_verts = W.shape[0]
        ...
        for pose in range(num_poses):
            for bone in range(num_bones):  
                p_corrected = rest_pose - rest_bones_t[bone] 
                constructed = np.empty((num_bones, num_verts, 3)) 
                for bone2 in range(num_bones):                  
                    Rp = bone_transforms[bone2,pose,:3,:].dot((rest_pose - rest_bones_t[bone2]).T).T                  
                    constructed[bone2] = Rp + bone_transforms[bone2, pose, 3, :]               
                constructed = constructed.transpose((1, 0, 2)) * W[:, :, np.newaxis] 
                constructed = np.delete(constructed, bone, axis=1)
                q = poses[pose] - np.sum(constructed, axis=1) 
                print np.sum(np.square(W[:, bone]))
                
        return bone_transforms

    def find_vtx_sixteen_Point(self , mesh , pos = [0,0,0]):
        ...
        nul = []
        cmds.select('{mesh}.f[{ID}]'.format(mesh = mesh, ID = ID))
        mel.eval('PolySelectTraverse 1;') # expend face
        lis_ = cmds.ls(sl=1,fl=1)
        for i in lis_:
            vtxSel = cmds.ls(cmds.polyListComponentConversion (i,ff=True,tv=True), fl=1)
            for y in vtxSel:
                y = y.split('[')[-1].split(']')[0] # y = index 
                if not y in nul:
                    nul.append(int(y))
    ...
    def addPoses(self):
        ...
        if self.ui.pose_LED.text() and self.ui.simulationRange_min_LED.text() and self.ui.simulationRange_max_LED.text(): 
    ...
    def updateBothFunc(self ):
        ...
        if self.mode == 1:
            ...
            W, bone_transforms, rest_bones_t ,self.finalBone = self.MTSC(self.poses, self.rest_pose , num_bones ,iterations ,maxIterations ,sparseness)
     
        if not self.finalBone:
            for x in rest_bones_t:
                    cmds.select(cl=1)
                    j = cmds.joint()
                    cmds.xform(j , t = x)
                    self.finalBone.append(j)
            for y in self.finalBone:
                cmds.select(y , add=1)                 
            cmds.skinCluster(self.finalBone , self.mesh , tsb = 1)    

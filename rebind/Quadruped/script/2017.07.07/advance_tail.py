...
class tailRigClass:
    ...
    def create_bind_joint( self, num ):
        ...
        cmds.select ( cl=1 )
        root_joint = cmds.joint ( p=joint_position, n='Skin_tail1_JNT' )
        ...
        for x in range(bindJointNum):
            ...
            cJoint = cmds.joint ( p=(joint_position[0], joint_position[1], Z_position ), n='Skin_tail%s_JNT' %(x+2) )
...
def tailOP():
    ...
    inter_num = int( cmds.textField ( 'bind_joint_num', q=1, tx=1 ) )
    ...
    if sel_type == 'normal':
        ...
        IKJoint_list = IS.create_bind_joint( inter_num )

...
class tailRigClass:
    ...
    def setting_controler(self):
        ...
        self.greate_controler( 'circle', 6, self.joint_list, 'tail' )
        self.greate_controler( 'sphere', 13, self.joint_list, 'tail' )
    ...
    def upAxis_control_rig(self, IKJoint_list, rigJoint_list ):
        ...
        for x in range( self.cycle_num ):
            ...
            cmds.delete ( cmds.parentConstraint ( self.circle_CON_list[x], each_up_LOC, mo=0 ) )
            cmds.parent ( each_up_LOC, self.circle_CON_list[x] )
        
        aim_vector = cmds.spaceLocator ( n='tail_aim_vector_LOC' )
        cmds.delete ( cmds.parentConstraint ( self.circle_CON_list[-1], aim_vector, mo=0 ) )
...
def tailOP():
    IS = tailRigClass()
    IS.create_curve()
    IKJoint_list = IS.create_joint( 'IK' )
    rigJoint_list = IS.create_joint( 'rig' )
    
    IS.setting_controler
    ...
    IS.upAxis_control_rig( IKJoint_list, rigJoint_list )
